...
class Users extends Component {
  ...
  secondCall = () => {
    ...
    axios
      .get(`https://api.unsplash.com/users/${user.username}/?client_id=${process.env.REACT_APP_API_KEY}`)
      .then(res => {
        const userphotos = res.data;
        const photos = res.data.photos;
        this.setState({userphotos: userphotos, photos: photos, isLoading: false});
      })
      .catch(err => console.log(err));
  }
        ...
        <div className="delay">
          <Card key={user.id}>
            ...
            <Button variant="body2" color="textSecondary" component="p">
              <a className="bisc" href={user.portfolio_url} target="_blank" rel="noopener noreferrer">
                Portfolio:
              </a>
            </Button>
